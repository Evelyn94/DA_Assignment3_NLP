...
def create_label_vec(label):
    ...
    label_vec[label_to_id[label[0:-1]]] = 1
    return label_vec


def tokenize(sens):
    # Tokenize a given sentence into a sequence of tokens.
    token_seq = nltk.word_tokenize(sens)
    return token_seq
...
def eval(word_to_id, train_dataset, dev_dataset, test_dataset):
    ...
    with tf.Session() as sess:
        ...
        sen_embed = tf.reduce_mean(embed, 0)
        x = tf.reshape(sen_embed, [1, embedding_dim])
        y = tf.nn.softmax(tf.matmul(x, W) + b)
        #y = tf.nn.softmax(tf.matmul(x, embeddings, transpose_b = True))
        cross_entropy = tf.reduce_mean(-tf.reduce_sum(correct_label * tf.log(y), reduction_indices=[1]))
        #cross_entropy = tf.nn.softmax_cross_entropy_with_logits(tf.log(y), correct_label)
        ...
        prediction = tf.cast(tf.argmax(y, 1), tf.float32)
        ...
        for epoch in range(num_epochs):
            ...
            #i = 0
            for (sen, lable) in train_dataset:
                #i = i+1
                train_step.run(feed_dict={input_sens: sen, correct_label: lable})
        ...
        test_results = predict(prediction, input_sens, correct_label, test_dataset)
    return test_results
...
def predict(prediction, input_sens, correct_label, test_dataset):
    test_results = []
    for (sens, label) in test_dataset:
        test_results.append(prediction.eval(feed_dict={input_sens: sens, correct_label: label}))
    return test_results
